...
Arrivé au level14, on cherche partout un fichier ou programme éxecuté par flag14 mais on trouve rien avec les commandes classiques.
Après réflexions, on commence à s'interesser au binaire de getfag. Même si c'est une commande mais plus important c'est un binaire 
donc on va l'étudier avec gdb.

- Début étude de getflag avec gdb
gdb getflag
break *main
run
disassemble
...
Plusieurs choses sont interessantes:
  - appel de ptrace: une protection contre gdb, il faudra sauter les etapes autours de ptrace pour eviter de sortir du programme.
  - appels à ft_des: Les appels à ft_des sont nombreux (autant que de nombres de flags). 
  En regardant les strings passées en paramètres, on peut en déduire que ft_des est une fonction de decryptage par des.
  - appel à getuid: il semble conditionner les strings récupérées avant les appels à ft_des.
  
Hors flag14 est un utilisateur donc il a un uid , on va le chercher dans /etc/passwd.
cat /etc/passwd
   -> flag14:x:3014:3014::/home/flag/flag14:/bin/bash
Uid flag14 = 3014

L'objectif va donc d'exploiter getflag en lui faisant croire que l'on est l'utilisateur flag14 par son uid.
Pour cela on va poser un break point juste apres l'appel de getuid. On retourne dans gdb, on run toujours avec le break sur le main.

Étape 1: éviter la sécurité ptrace
Pour éviter les problèmes avec ptrace, on break juste après son appel au niveau de la comparaison:
...
Il vaut donc -1 donc passer la comparaison, on va passer sa valeur à 0:

  print $eax=0x0

Ça fonctionne on peut passer à l'étape suivante.

Étape 2: modifier l'uid
On arrive sur notre breakpoint 2 après l'appel de getuid. On affiche sa valeur

   info registers
   eax            0x7de    2014

Donc il faut modifier eax avec l'uid de flag14 qui 3014 soit 0xbc6 en hexadecimal
    ...
    [Inferior 1 (process 2667) exited normally]
